...
export const getCheckoutPageData = async (userId, appliedCouponId) => {
    ...
    const nonDiscountedTotal = validItems.reduce((sum, item) => sum + (item.nonDisPrice * item.quantity), 0);
    ...
    const addressData = await Address.find({ user_id: userId }).sort({ is_default: -1 });
...
export const placeCODOrder = async (userId, addressId, couponDiscount, couponId) => {
    const { totalAmount } = await calculateCartDetails(userId);
    const finalPrice = totalAmount - couponDiscount;

    if (finalPrice < 1000) {
        throw new Error("Cash on Delivery (COD) is available for orders ₹1000 and above.");
    }

    const order = await createOrderDocument(userId, addressId, "COD", "Pending", couponDiscount, couponId);
...
export const placeWalletOrder = async (userId, addressId, couponDiscount, couponId) => {
    ...
    if (wallet.balance < finalPrice) throw new Error("Insufficient wallet balance.");

    const order = await createOrderDocument(userId, addressId, "Wallet", "Paid", couponDiscount, couponId);
    ...
    try {
        ...
        throw new Error(paymentError.message || "Payment failed after order creation.");
    }
};
...
export const verifyAndPlaceRazorpayOrder = async (userId, paymentData, couponDiscount, couponId) => {
    ...
    const order = await createOrderDocument(userId, addressId, paymentMethod, "Paid", couponDiscount, couponId);
    await internalFinalizeOrder(userId, order.items);

    return order;
};

export const handleFailedPaymentOrder = async (userId, addressId, paymentMethod, couponDiscount, couponId) => {
    
    const order = await createOrderDocument(userId, addressId, paymentMethod, "Failed", couponDiscount, couponId);
