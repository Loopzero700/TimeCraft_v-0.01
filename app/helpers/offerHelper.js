...
const applyOffersToProduct = (product, activeOffers) => { 
    ...
    const matchingProductOffers = activeOffers.filter(offer => 
    ...
    const matchingCategoryOffers = activeOffers.filter(offer => 
    ...
    product.variants.forEach(variant => {
        ...
        if (bestOffer) {
            const offerPrice = Math.round(basePrice - (basePrice * maxDiscountPercent / 100))
