...
const checkAndUpdateOrderStatus = async (orderId) => {
  try {
    ...
    const allReturnedOrCancelled = itemStatuses.every(
      (status) => status === "Return-Approved" || status === "Cancelled"
    );

    const hasAtLeastOneReturned = itemStatuses.some(
...
export { checkAndUpdateOrderStatus };
