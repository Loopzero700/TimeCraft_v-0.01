...
const getSalesData = asynchandler(async (req, res) => {
    try {
        const { page, limit, search, filterType, startDate, endDate } = req.query
        ...
        const stats = summaryResult[0] || { totalOrders: 0, totalRevenue: 0, totalDiscount: 0 }
...
const downloadSalesExcel = asynchandler(async (req, res) => {
    try {
        const { filterType, startDate, endDate, search } = req.query
        ...
        const totalRow = worksheet.addRow(['', '', '', '', 'Total Revenue:', `Rs. ${totalRevenue}`])
...
const downloadSalesPDF = asynchandler(async (req, res) => {
    try {
        ...
        orders.forEach((order, index) => {
            if (currentY > 720) { doc.addPage(); currentY = 30; }
            ...
            doc.text(new Date(order.createdAt).toLocaleDateString('en-GB'), colX[1], currentY + 8)
            doc.text((order.address_name || 'N/A').substring(0, 20), colX[2], currentY + 8)
            doc.text(`Rs. ${order.total.toLocaleString('en-IN')}`, colX[3], currentY + 8)
        ...
        doc.moveTo(summaryX + padding, summaryY + 25).lineTo(summaryX + boxWidth - padding, summaryY + 25).stroke()
        ...
        const drawMetric = (label, value, y, isBold = false) => {
            ...
            doc.text(value, summaryX + padding, y, { align: 'right', width: boxWidth - (padding * 2) })
        }

        drawMetric('Total Sales Count:', orders.length.toString(), metricStartY)
        drawMetric('Total Discount:', `Rs. ${totalDiscount.toLocaleString('en-IN')}`, metricStartY + gap)
        
        doc.rect(summaryX, metricStartY + (gap * 2) - 5, boxWidth, 25).fill('#e5e7eb')
        doc.fill('#000000')
        drawMetric('Total Revenue:', `Rs. ${totalRevenue.toLocaleString('en-IN')}`, metricStartY + (gap * 2), true)
